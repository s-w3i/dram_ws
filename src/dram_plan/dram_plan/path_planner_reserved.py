...
class PathPlanner(Node):
    def __init__(self):
        ...
        # Subscribers
        self.available_resources_sub = self.create_subscription(
            Graph,
            '/available_resources',
            self.available_resources_callback,
            10
        )
        ...
        self.reserved_nodes_sub = self.create_subscription(
            Int32MultiArray,
            '/reserved_nodes',
            self.reserved_nodes_callback,
            10
        )
        ...
        self.available_graph = None  # Available resources graph
        self.robot_states = {}        # {robot_name: (x, y)}
        self.reserved_nodes = set()   # Set of reserved node indices
    ...
    def available_resources_callback(self, msg: Graph):
        """
        Callback to receive the available resources graph.
        This graph should contain all nodes and edges that are currently unreserved.
        """
        self.available_graph = msg
        self.get_logger().info(f"Received available resources with {len(msg.vertices)} vertices and {len(msg.edges)} edges.")

    def reserved_nodes_callback(self, msg: Int32MultiArray):
        """
        Callback to receive the list of reserved node indices.
        """
        self.reserved_nodes = set(msg.data)
        self.get_logger().debug(f"Updated reserved nodes: {self.reserved_nodes}")
    ...
    def attempt_path_computation(self, robot_name):
        ...
        if robot_name not in self.pending_requests:
            self.get_logger().error(f"No pending request found for robot {robot_name}. Ignoring retry.")
            return
        ...
        # Check if available_graph is ready
        if not self.available_graph:
            response.success = False
            response.message = "Available resources not received."
            self.get_logger().error(response.message)
        ...
        # Find the goal vertex index in the available graph
        goal_idx = self.find_vertex_index_by_name(request.goal_vertex_name, self.available_graph)
        if goal_idx is None:
            response.success = False
            response.message = f"Goal vertex '{request.goal_vertex_name}' not found in the available resources."
            self.get_logger().error(response.message)
            self.cleanup_pending_request(robot_name)
            return

        # Check if the goal node is reserved
        if goal_idx in self.reserved_nodes:
            response.success = False
            response.message = f"Goal vertex '{request.goal_vertex_name}' is reserved."
            self.get_logger().error(response.message)
            self.cleanup_pending_request(robot_name)
            return

        # Find the closest available node to the robot's current position
        current_pos = self.robot_states[robot_name]
        start_idx = self.find_closest_node(current_pos, self.available_graph)
        ...
        # Check if the start node is reserved
        if start_idx in self.reserved_nodes:
            response.success = False
            response.message = f"Start node near robot {robot_name}'s position is reserved."
            self.get_logger().error(response.message)
            self.cleanup_pending_request(robot_name)
            return

        # Compute the shortest path using Dijkstra's algorithm on the available graph
        path_indices = self.compute_shortest_path(start_idx, goal_idx, self.available_graph)
        ...
        if path_indices:
            ...
            for idx in path_indices:
                node = self.available_graph.vertices[idx]
                location = Location()
                location.x = node.x
                location.y = node.y
                location.yaw = 0.0  # Assuming no yaw; modify if necessary
                location.level_name = self.available_graph.name  # Assuming all nodes are on the same level
    ...
    def compute_shortest_path(self, start_idx, goal_idx, graph: Graph):
        ...
        # Build adjacency list from available_resources graph
        ...
        while queue:
            ...
            if current in self.reserved_nodes:
                continue
            ...
            for neighbor in adjacency[current]:
                if neighbor in visited or neighbor in self.reserved_nodes:
                    continue
                new_cost = cost + sqrt(
